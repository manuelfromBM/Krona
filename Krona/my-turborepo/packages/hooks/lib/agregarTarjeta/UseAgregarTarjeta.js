...
export const useAgregarTarjeta = () => {
    ...
    const onChangeRut = (text) => setRut(text.replace(/[^0-9kK]/g, "").slice(0, 12));
    const handleGuardar = () => {
        if (!cardName.trim())
            return Alert.alert("Error", "Ingresa el nombre del titular.");
        if (cardNumberRaw.length < 13)
            return Alert.alert("Error", "Número de tarjeta incompleto.");
        if (expiryRaw.length !== 4)
            return Alert.alert("Error", "Fecha de expiración inválida.");
        if (cvv.length < 3)
            return Alert.alert("Error", "CVV inválido.");
        if (!rut.trim())
            return Alert.alert("Error", "Ingresa RUT.");
...
//# sourceMappingURL=UseAgregarTarjeta.js.map